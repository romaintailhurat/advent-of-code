...
// {1: {1:true}, 3:{2:true}} if [1,1] and [3,2] visited
var positionStore = {};
...
function vectorAfterMove(currentVector, move) {
  let m = MOVES[move];
  return vectorAdd(currentVector, m);
}

function storePosition(vector) {
  if(vector.length !== 2) { throw new Error('bam!'); }
  let x = vector[0];
  var y = vector[1];
  if(!positionStore[x]) {
    positionStore[x] = {};
    positionStore[x][y] = true;
  } else if (!positionStore[x][y]) {
    positionStore[x][y] = true;
  }
}

function countPosition(positions) {
  var count = 0;
  Object.keys(positions).forEach((k,i) => {
    Object.keys(positions[k]).forEach((l, i) => {
      count += 1;
     });
  });
  return count;
...
function vectorAdd(v, w) {
  if(v.length !== w.length) { throw new Error('bim!'); }
  return v.map((e,i) => v[i] + w[i]);
}

var res = start;
storePosition(res);
input.split('').forEach((e, i) => {
  res = vectorAfterMove(res, e);
  storePosition(res);
  //console.log('after move', res);
});
console.log('Position store', positionStore);
console.log(countPosition(positionStore));